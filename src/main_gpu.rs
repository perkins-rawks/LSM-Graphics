...
// // curve)
// fn cluster(window: &mut Window, n: usize, r: f32, loc: &Point3<f32>, var: f32) {
//     // StreamDraw means we will constantly change the vector
//     // Dynamic means we will change it decently often
//     // Sphere_Data
//     let mut sp_data = GPUVec::new(Vec::new(), BufferType::ElementArray, AllocationType::DynamicDraw);
...
//         // push the radius, location onto the locations vector
//         // create the sphere from the location / radius
//         // change its color
//         let data: Vector4<f32> = Vector4::new(r, x, y, z);
//         if let Some(sp_data) = sp_data.data_mut() {
//             sp_data.push(data);
//             // sp_data[sphere].set_color(color.0, color.1, color.2);
//             // sp_data[sphere].append_translation(&t);
//         }
//         sp_data.load_to_gpu();
...
//     // For each sphere in the sphere data GPU vector, create a sphere
//     for sp_d in sp_data.data_mut().as_ref().unwrap().iter() {
//         let mut sphere = window.add_sphere(sp_d.x);
//         sphere.set_color(color.0, color.1, color.2);
//         sphere.append_translation(&Translation3::new(sp_d.y, sp_d.z, sp_d.w));
//     }
//     println!("{}", sp_data.is_on_gpu());
//     // sp_data.unload_from_gpu();
// }
...
//     cluster(&mut window, n, r, &c1, var);

//     n = 500;
//     var = 1.5;
//     let c2 = Point3::new(5., -1.3, -3.4);
//     cluster(&mut window, n, r, &c2, var);

//     let c3 = Point3::new(3., 6., 1.);
//     cluster(&mut window, n, r, &c3, var);
