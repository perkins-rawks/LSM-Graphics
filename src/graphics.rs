/*
    Rendering of the window and all its nuances with keybinds are handled here. 

    Most graphical things are made from the crate kiss3d linked here: 
    https://docs.rs/kiss3d/0.24.1/kiss3d/
*/
...
pub fn render_lines(
    ...
    time_steps: u32,                                              // The total number of time steps in an epoch of signals
    conditions: [bool; 8], // conditions is a list containing all of the initial graphical settings. 
    ...
    // of white to black, only if we have it activated
    let mut connect_sp: Vec<SceneNode> = Vec::new();
    if fancy {
        connect_sp = fancy_lines(window, lines, dists);
    }

    let mut time_step = 0; // used in displaying spiking later on
    ...
    while window.render_with_camera(&mut arc_ball) {
        ...
        for n_idx in 0..n_total {
            ...
            // Since we don't want it to spike (graphically) for very long
            if time_step / speed > spike_len {
                let past_spike_time = l.get_neurons()[n_idx]
                .get_spike_times()
                .contains(&((time_step / speed) - spike_len));
                // If the current neuron spiked before, set it back to original color
                if past_spike_time {
            ...
            // If the current neuron is currently spiking, set it to red
            if curr_spike_time {
                l.get_neurons()[n_idx].get_obj().set_color(1., 0., 0.);
            }

            // Resetting at a certain time
            if time_step / speed + spike_len + 1 > time_steps {
                time_step = 0;
            }
        }

        // Toggles the liquid neurons
        if !sp_on {
            sp_show(l);
            sp_on = !sp_on;
        }

        // Toggles readout neurons
        if !readout_on {
            readout_show(l);
            readout_on = !readout_on;
        }

        // Toggles fancy connection showing in the liquid with spheres
        if !fancy {
...
fn fancy_lines(
    ...
    let sp_per_dist = 0.03_f32; // 1 sphere every 0.03 units
